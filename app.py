...
def calculate_hit_rate(df, history_count=10):
    ...
    return (
        round(hit_1 / total * 100, 2),
        round(hit_2 / total * 100, 2)
    )

# =========================
# 动态学习模型
# =========================
def dynamic_learn(df):
    ...
    wave_counter = Counter(special_waves)

    zodiac_counter = Counter(special_zodiac)

    tail_counter = Counter(special_tails)

    # 波色连续出现降温
    for w, c in wave_counter.items():

        if c >= 5:
            LEARN_WEIGHT["波色"] -= 1

    # 生肖连续出现降温
    for z, c in zodiac_counter.items():
    ...
    for _, row in recent.iterrows():
        ...
        for i in range(len(nums)-1):
            ...
            if diff <= 2:
                ...
                combo_score[nums[i]] += 2

                combo_score[nums[i+1]] += 2
