    ...
    if learning:
        try:
            if len(df) > history_count + 2:
                ...
                # 关键：这里截取时不使用自身的递归，直接用当前函数的中间预测结果计算模拟命中
...
result = ai_engine(df, history_count, learning=True)

# 融合增强（您的等比缩放版）
for n in result["prob"]:
    boost = ensemble_score[n] / 100
    result["prob"][n] = round(result["prob"][n] + boost, 2)
