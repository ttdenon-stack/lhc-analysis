...
def ensemble_predict(df):
    ...
    for window,weight in windows:
        ...
        result = ai_engine(df,window)

        for idx,n in enumerate(result["numbers"]):

            score[n] += (12-idx)*weight

        for idx,n in enumerate(result["special"]):

            score[n] += (8-idx)*weight*1.5
...
# 多窗口融合最终推荐
# =====================================================
ensemble_numbers = ensemble_predict(df)

result["numbers"] = ensemble_numbers
...
for i in range(15):
    ...
    try:
        ...
        predict=ai_engine(test_df,12)

        pred_nums=set(predict["numbers"][:10])

        real_nums=set(df.iloc[i]["nums"])

        hit=len(pred_nums & real_nums)
